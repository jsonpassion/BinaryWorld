# Binary World

> Everything is made of 0s and 1s.

Currently under review for **iPad** and **macOS** on the App Store. Release date **TBA**.

| App Icon |
|:---:|
| [![App Icon](screenshots/icon.png)](https://apps.apple.com/kr/app/binary-world-ssc/id6761305723?l=en-GB) |
| Available on macOS and iPadOS only |

---

**Binary World** is an interactive iPad app where you experience how computers convert numbers, text, and images into binary (0s and 1s) — through visual, hands-on gameplay designed for anyone, no coding background needed.

This app was my first challenge at the **Apple Swift Student Challenge 2026**, where I was selected as a **Winner**.

---

## What You'll Experience

### Mission 1 — Digital Clock
Tap a digit on a clock. Watch it break apart into 7 light segments, then discover **BCD** — the 4-bit binary code behind every digit. Toggle the bits yourself to set the time.

### Mission 2 — Text Message
A chat reveals a secret: computers can't read letters. See "Hi" decompose into **ASCII** codes, then into raw 0s and 1s. Decode a hidden 3-letter message byte by byte.

### Mission 3 — Pixel Art
A heart image pixelates step by step into an 8x8 binary grid. Each pixel is just a single bit. Complete pixel art by reading binary hints and filling in the blanks.

---

## Screenshots

| Main Lobby | Clock | Message | Pixel |
|:---:|:---:|:---:|:---:|
| ![lobby](screenshots/lobby.png) | ![clock](screenshots/clock.png) | ![message](screenshots/message.png) | ![pixel](screenshots/pixel.png) |

---

## About

| **Platform**  | **Built with** | **Data Collection** |
|:---:|:---:|:---:|
| iPad only (iPadOS 26+) | SwiftUI | None |

---

## Privacy

[Privacy Policy](privacy.md)

---

Built by [Jason J. Lee — Tech Mentor at Apple Developer Academy POSTECH](https://www.linkedin.com/in/json-lee)
